...
#
# 0.5.11.1
# station kickstart was seperated from the server kickstart because it interfered with the processing of the server install
# The station kickstart is now imported from the floppy at the end of the server kickstart, and the variables are substituted at that time
# There is now an rm command to remove /etc/yum.repos.d/*.repo before placing the server1 repo
# this is to remove the default internet repos. The station won't have internet, and we want to version freeze it to the server repo version.
#
# 0.5.11.2
# make dir /etc/openldap/cacerts, since it is not auto created by CentOS (it is with RHEL)
...
############################################################
echo "$distro_name Lab in a Box - server1 kickstart 0.5.11.1" >/etc/kickstart-release
